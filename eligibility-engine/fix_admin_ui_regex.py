...
# I will find the entire renderAgeLimits function and replace it surgically.
# The function starts with 'const renderAgeLimits = () => {'
# The function ends with ');\\n  };\\n\\n  const renderJobTypeSection'

import re

# Refined regex for the entire function
                    ...
                    if (hasExemptionGroup) {
                      return (
                        <div style={{ display: "flex", alignItems: "center", gap: "6px", background: "rgba(255,255,255,0.6)", padding: "4px 10px", borderRadius: "8px", border: "1px solid var(--border-subtle)" }}>
                          ...
                          <input type="number" style={{ width: "28px", background: "transparent", border: "none", fontSize: "0.85rem", fontWeight: 800, padding: 0, textAlign: "right", color: "var(--accent-primary)" }} value={activeExam.marital_grace_period ?? 5} onChange={(e) => updateExamData((p) => ({ ...p, marital_grace_period: Number(e.target.value) }))} />
...
if re.search(pattern, text):
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(new_text)
    print("Successfully replaced renderAgeLimits")
else:
